...
-- Discord self-registration uses this server-only function so one button flow
-- can create the roster member, Discord link, and its initial safe status row.
-- Existing projects can run supabase/discord-registration.sql instead.
